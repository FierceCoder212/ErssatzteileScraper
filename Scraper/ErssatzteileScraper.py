...
class ErssatzteileScraper:
    ...
    def scrape_url(self, scraper_data: ScraperDataModel) -> CatalogModel:
        while True:
            response = requests.get(scraper_data.catalog_link, headers=self.headers)
    ...
    def translate_data(self, data: list[str]):
    ...
    def get_scraper_data(self, file_path: str) -> list[ScraperDataModel]:
        with open(file_path, 'r') as scraper_data_file:
            scraper_data = json.load(scraper_data_file)
        return [ScraperDataModel(**data) for data in scraper_data if data['sgl_code'] not in self.prev_data]

    @staticmethod
    def get_json(file_path: str) -> list:
